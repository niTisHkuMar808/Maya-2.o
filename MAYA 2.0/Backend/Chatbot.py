...
def RealtimeInformation():
    ...
    hour = current_date_time.strftime("%H") # Hour in 24-hour formati
...
def ChatBot(Query):
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            temperature = 0.7, #Adjust response randomness (bigher means more random).
            top_p=1, #Use nucleus sampling to control diversity.
            stream = True, #Enable streaming response.
            stop = None #Allow the model to determine when to stop,
        )
        Answer = "" #Initialize an empty string to store the Al's response.

        #Process the streamed response chinks.

        for chunk in completion:           
            if chunk.choices[0].delta.content: #Check if there's content in the current chunk.
                Answer += chunk.choices[0].delta.content #Append the content to the answer.

        Answer = Answer.replace("</s>", "") #Clean up soy unwanted tokens from the response.

        #Append the chatbot's response to the messages list.  
        messages.append({"role": "assistant", "content": Answer})

        #Save the updated chat log to the 350N file.

        with open(r"Data\ChatLog.json", "w") as f:
            dump(messages, f, indent=4)

        #Return the formatted response. return AnswerModifier(Answer Answer)
       ...
       return ChatBot(Query) #Retry the query after resetting the lag.
